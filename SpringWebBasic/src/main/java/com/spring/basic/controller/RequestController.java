...
public class RequestController {
	...
	@RequestMapping("/test") // /request/test라는 요청이 들어오면 testCall()메소드를 불러줘
...
//	@RequestMapping(value="/request/basic01", method = RequestMethod.GET) spring 3버전까지 문법
	@GetMapping("/basic01")	
...
//	@RequestMapping(value="/request/basic01", method = RequestMethod.POST)
	...
	@GetMapping("/join")
	public void register() { // URL보고 리턴할 곳 결정 (URL을 파일 경로로 사용)
	...
	*/
	...
	public void register(
			String userId, // 선언한 변수명과 파라미터 값이 동일하면 @RequestParam("userId") 안써도 됨
			String userPw,
			@RequestParam(value="hobby", required = false, defaultValue = "no hobby person") List<String> hobby
	) {						// required: 값이 없을때 에러발생은 true, 에러발생 안나게하기 false, defaultValue: 값 없을때 출력되는 내용
