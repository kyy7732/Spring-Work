...
public class MailSenderService {
	...
	public String joinEmail(String email) {
		int authNum = makeRandemNumber(); // 인증번호 생성
		
		String setFrom = "kyy77321@naver.com"; // email-config에 설정한 발신용 이메일 주소 입력.
		...
		String content = "홈페이지 가입을 신청해 주셔서 감사합니다." +
						...
						"<br>" +
						"해당 인증 "
						+ "번호를 인증번호 확인란에 기입해 주세요."; // 이메일에 삽입할 내용.
		
//		mailSend(setFrom, toMail, title, content); 메일 전송
	...
	private void mailSend(String setFrom, String toMail, String title, String content) {
		...
		try {
			...
			//생성자의 매개값으로는 MimeMessage 객체, bool(첨부파일 여부), 문자 인코딩 설정
			...
			// true -> html 형식으로 전송(content값에 html형식으로 작성했기때문), 값을 안주면 단순 텍스트로 전달.
