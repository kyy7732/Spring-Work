            ...
            <div class="row">
                <div class="col-xs-12 col-md-9 write-wrap">
                        ...
                        <form action="${pageContext.request.contextPath}/freeboard/modPage" method="post">
                            <div>
                                <label>DATE</label>
                                <p>${boardContent.date}</p>
                            </div>   
                            <div class="form-group">
                                <label>번호</label>
                                <input class="form-control" name="bno" value="${boardContent.bno}"readonly>
                            </div>
                            <div class="form-group">
                                <label>작성자</label>
                                <input class="form-control" name="writer" value="${boardContent.writer}" readonly>
                            </div>    
                            <div class="form-group">
                                <label>제목</label>
                                <input class="form-control" name="title" value="${boardContent.title}" readonly>
                            </div>

                            <div class="form-group">
                                <label>내용</label>
                                <textarea class="form-control" rows="10" name="content" readonly>${boardContent.content}</textarea>
                            </div>
                ...
                <div class="row">
                    <div class="col-xs-12 col-md-9 write-wrap">
	                    ...
	                    <div class="reply-content">
	                        <textarea class="form-control" rows="3" id="reply"></textarea> <!-- 글 내용 -->
	...
	<script>
        window.onload = function() {
            ...
            document.getElementById('replyRegist').onclick = () => {
                ...
                const bno = '${boardContent.bno}'; // 현재 게시글 번호도 보내야 한다.
                ...
                fetch('${pageContext.request.contextPath}/reply', reqObj)
                    .then(res => res.text()) // bno, replyText, replyId, replyPw 등 값이 있음
                    .then(data => {
            ...
            function getList(pageNum, reset) {
                ...
                const bno = '${boardContent.bno}'; // 현재 게시글 번호

                // get방식으로 댓글 목록을 요청 (비동기)
                fetch('${pageContext.request.contextPath}/reply/list/' + bno + '/' + pageNum)
                    .then(res => res.json()) // map이오고 json으로 변환되어서 와서 json달라고 함
                    .then(data => { // total과 list가 들어있음
                        ...
                        if(reset) {
                            while($replyList.firstChild) {// while문에 null이 오면 false로 해석된다. (0, 빈 문자열 등등)
                        ...
                        if(!reset) { //reset이 true면 맨 위쪽에 추가 , false면 아래쪽부터 추가
                            $replyList.insertAdjacentHTML('beforeend', strAdd); // 삽입할 위치와, 삽입할 값을 받는다
                        } else {
                            // innerHTML 같은 경우는 
            ...
            // 수정, 삭제
            /*
            document.querySelector('.replyModify').onclick = function(e) { // 이벤트가 발생을 안함
                e.preventDefault(); // a태그 동작못하게 하기
            ...
            getList라는 함수를 통해서 만들어지고 처음부터 만들어진 객체가 아니기때문에 fetch가 언제 만들어질지
            모르기 때문에 onclick이라는 이벤트가 걸리질 않는다.

            그렇기 때문에 getList를 만드는곳인  div 태그의 id인 replyList에 이벤트 전파방식인 addEventListener를 사용한다.
            */
            document.getElementById('replyList').addEventListener('click', e => {
                ...
                if(!e.target.matches('a')) return; // 현재 a태그가 2개여서 a라고 쓰지만 a태그가 각각 사용된다면 a.class이름이나 id를 사용한다
                
                // 2. a태그가 두 개(수정, 삭제)이므로 어떤 링크인지를 확인.
                // 댓글이 여러 개 -> 수정, 삭제가 발생하는 댓글이 몇 번인지도 확인해야 한다.

                const rno = e.target.getAttribute('href'); // 댓글번호 얻기
                ...
                if(e.target.classList.contains('replyModify')) { // classList: arrayList를 통해 모든 클래스 이름을 받아옴
                    ...
                    $('#replyModal').modal('show'); // 숨겨진 모달 나옴
            ...
            document.getElementById('modalModBtn').onclick = () => {
                ...
                // 요청에 관련된 정보 객체
                const reqObj = {
                    method: 'put', // put: 요청방식만 보고도 알 수 있게?
                    headers: {
            ...
            */
            ...
            const rno = document.getElementById('modalRno').value; // 모달 rno hidden으로 숨길 번호
            const replyPw = document.getElementById('modalPw').value; // 모달 pw 비밀번호 input창 값
            ...
            const reqObj = {
                    ...
                    body: JSON.stringify({ 
                        // body: (replyPw) 객체 1개만 보낼때는 이런식으로 사용 가능 
            ...
            function parseTime(regDate) {
                ...
                const gap = now.getTime() - regTime.getTime(); // ex) 16:58(now) - 16:46(regTime) 
                
                let time;
                if(gap < 60 * 60 * 24 * 1000) { // 하루 전
                    if(gap < 60 * 60 * 1000) { // 한시간 전
                        time = '방금 전';
                    } else {
                        time = parseInt(gap / (1000 * 60 * 60)) + '시간 전';
                    }

                } else if(gap < 60 * 60 * 24 * 30 * 1000) { // 한달 지남
                    time = parseInt(gap / (1000 * 60 * 60 * 24)) + '일 전';
                } else {
                    time = `\${regTime.getFullYear()}년 \${regTime.getMonth()+1}월 \${regTime.getDate()}일`; // 0월부터 시작하므로 +1을 달아줌
                }
