...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>  
                        ...
                        <tbody>
                        	<c:forEach var="vo" items="${boardList}">
	                            <tr>
	                                <td>${vo.bno}</td>
	                                <td>
	                                <a href="${pageContext.request.contextPath}/freeboard/freeContent?bno=${vo.bno}&pageNo=${pc.page.pageNo}&amount=${pc.page.amount}&keyword=${pc.page.keyword}&condition=${pc.page.condition}">${vo.title}</a>
	                                </td>
	                                <td>${vo.writer}</td>
	                                <td>${vo.date}</td>
	                            </tr>
    ...
    <script>
        ...
        window.onload = function() {
            ...
            // e => : 이벤트가 발생했을때 수행할 메서드
            document.getElementById('pagination').addEventListener('click', e => {
                if(!e.target.matches('a')) {
                    return;
                }

                e.preventDefault(); // a태그의 고유 기능 중지 href='#' 중지
