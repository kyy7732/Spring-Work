...
<script>
    let code = ''; // 이메일 전송 인증번호 저장을 위한 변수 const는 값을 못 바꿈
    ...
    document.getElementById('idCheckBtn').onclick = () => {
        ...
        */
        ...
        .then(res => { // 먼저 진행 후 다음 then실행
            ...
            // promise가 then이라는 객체를 갖고있음
            // 요청이 완료된 후 then()의 매개값으로 응답에 관련된 함수를
            // 콜백(다른 함수의 파라미터로써 전달되는 함수를 콜백 함수) 방식으로 전달합니다. (function(res) -> 화살표 함수로 간단히 표현)
        ...
        fetch('${pageContext.request.contextPath}/user/id/' + userId)
            ...
            .then(data => { // 텍스트 데이터만 담긴 Promise 객체로부터 data를 전달받음.
                if (data === 'ok') {
                    ...
                    true); // userId에 readonly의값을 true로 변환
    ...
    document.getElementById('mail-check-btn').onclick = function() {
        ...
        .then(data => {
            ...
            // 비활성화된 인증번호 입력창을 활성화
            document.querySelector('.mail-check-input').disabled = false; // 인증번호가 전달되었을때 입력가능하게
    ...
    document.querySelector('.mail-check-input').onblur = function(e) {
        ...
        if(inputCode === code) {
            ...
            // select는 readonly가 없어서 이런 로직으로 처리를 한다.
            $email2.setAttribute('onFocus', 'this.initialSelect = this.selectedIndex');
            $email2.setAttribute('onChange', 'this.selectedIndex = this.initialSelect'); // 사용자가 옵션을 바꾸려고 하면 초기 설정값으로 돌아간다
        } else {
    ...
    function searchAddress() {
        new daum.Postcode({
            oncomplete: function(data) {
                ...
                if (data.userSelectedType === 'R') { // 사용자가 도로명 주소를 선택했을 경우 userSelectedType이 R로 선택된다
    ...
    document.getElementById('joinBtn').onclick = function() {
        if(idFlag && pwFlag) {
            if(!document.getElementById('userId').getAttribute('readonly')) { // readonly가 false라면 중복체크를 안한거다
