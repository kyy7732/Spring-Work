...
public class FreeBoardControllerTest {
    ...
    */
	...
	private WebApplicationContext ctx; // 가상의 DispatcherServlet을 사용하기위해 작성
...
//		this.mockMvc = MockMvcBuilders.standaloneSetup(controller).build();
	...
	void testModify() throws Exception {
		String bno = "5";
		String viewModify = mockMvc.perform(MockMvcRequestBuilders.post("/freeboard/freeModify")
																		.param("bno", bno)
																		.param("title", "수정")
																		.param("content", "수정수정")
									).andReturn().getModelAndView().getViewName();
		assertEquals(viewModify, "redirect:/freeboard/freeContent?bno=" + bno);
